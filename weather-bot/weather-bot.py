def bot(
    ...
    is_12_45_mt = mt_time.hour == 12 and mt_time.minute >= 45 and mt_time.minute < 50  # 5-minute window
    
    # Manual trigger: exactly "weather" (case-insensitive)
    is_weather_trigger = message_text.strip().lower() == "weather"
    
    # Auto-trigger: any message at 12:45pm MT (to allow scheduled posts)
    is_time_trigger = is_12_45_mt and message_text.strip() != ""
    
    # Respond if it's time trigger OR manual weather trigger
    if is_time_trigger or is_weather_trigger:
        if is_time_trigger:
            print(f"*** BOT RESPONDING - TIME TRIGGER: 12:45pm MT ***")
        else:
